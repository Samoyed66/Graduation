<template>
  ...
  <div id="Dance">
    ...
    <ul class="subNav clear">
      ...
      <router-link tag="li" to="/sports">体育</router-link>
      <router-link tag="li" to="/dance">舞蹈古典</router-link>
      <router-link tag="li" to="/children">亲子</router-link>
      <router-link tag="li" to="/rock">摇滚</router-link>
      <router-link tag="li" to="/drama">戏剧</router-link>
    </ul>
    <!-- 导航 -->
    <!-- 轮播图 -->
...
export default {
  ...
  methods: {
    ...
    /* 精彩聚集初始化数据 */
    focusingFn () {
      this.$http.get(this.$url + 'danceFocus').then((res) => {
        this.focusing = res.data.focus[0]
      })
    },
    /* 猜我喜欢初始化数据 */
    LikeFn () {
      this.$http.get(this.$url + 'danceLike').then((res) => {
    ...
    // 选项卡初始化数据
    cardListFn () {
      this.$http.get(this.$url + 'danceCard').then((res) => {
        this.cardList = res.data.card
      })
    },
    // 热门推荐初始化数据
    HotListFn () {
      this.$http.get(this.$url + 'danceHot').then((res) => {
